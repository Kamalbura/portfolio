...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadProjects() {
        ...
        try {
            ...
            if (!projects || projects.length === 0) {
                // Fallback: fetch from projects.json with cache-busting parameter
    ...
    function sortProjects() {
        if (!state.filteredProjects || !Array.isArray(state.filteredProjects)) {
            console.error('Cannot sort projects: filteredProjects is not an array', state.filteredProjects);
    ...
    function filterProjects() {
        ...
        if (!allProjects || !Array.isArray(allProjects)) {
            console.error('Cannot filter projects: allProjects is not an array', allProjects);
            state.filteredProjects = [];
            renderProjects();
            return;
        }
        ...
        state.filteredProjects = allProjects.filter(project => {
            ...
            if (searchTerm) {
                const technologies = project.technologies && Array.isArray(project.technologies) ? 
                    project.technologies.join(' ') : '';
                const searchContent = `${project.name} ${project.description} ${technologies}`.toLowerCase();
                searchMatches = searchContent.includes(searchTerm.toLowerCase());
            }
            
            // Filter out personal projects from main projects section
            // if they have the personal:true flag
            const isPersonalProject = project.personal === true;
            
            return categoryMatches && searchMatches && !isPersonalProject;
        });
    ...
    function renderProjects() {
        try {
            ...
            if (!state.filteredProjects || state.filteredProjects.length === 0) {
                projectsGrid.innerHTML = `
            ...
            renderErrorState('Error rendering projects. Please reload the page.');
        }
    }
                ...
                categoryButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        try {
                            ...
                            console.error('Error in category click handler:', err);
                        }
                    });
                });
            }
